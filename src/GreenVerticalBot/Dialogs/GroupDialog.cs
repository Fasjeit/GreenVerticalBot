...
namespace GreenVerticalBot.Dialogs
{
    internal class GroupDialog : DialogBase
    {
        ...
        {
            // свой try, чтоб не кидать в общий чат какие либо ошибки
            try
            {
                ...
                if (update?.Message?.Text is string { } text)
                {
                    ...
                    {
                        await this.DeleteCommandMessageAsync(botClient, cancellationToken);
                    ...
                    else if (text.StartsWith("/authenticate"))
                    {
                        ...
                        if (chatInfo == null)
                        {
                            ...
                            //catch (Exception ex)
                            //{
                            //    this.Logger.LogError($"cannot send info message to user [{this.Context.TelegramUserId}]: [{ex}]");
                            //}
                            return;
                        }
                            ...
                            TransactionScopeAsyncFlowOption.Enabled))
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        this.Logger.LogError($"cannot send confirmation message to user [{user.TelegramId}]: [{ex}]");
                                    }
                                    return;
                                }
                            }
            ...
            catch (Exception ex)
            {
                this.Logger.LogError($"user [{StringFormatHelper.GetUserIdForLogs(update!)}] " +
                          $":group dialog error [{ex.ToString()}]");
            }
        }

        internal async Task DeleteCommandMessageAsync(ITelegramBotClient botClient, CancellationToken cancellationToken)
        {
